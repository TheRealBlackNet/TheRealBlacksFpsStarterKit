...
@onready var shader_layer: MeshInstance2D = %ShaderLayer
...
func _ready():
	...
	shader_layer.mesh.size = Vector2(\
		camera_3d.get_viewport().size)
	shader_layer.mesh.center_offset = Vector3(\
		camera_3d.get_viewport().size.x / 2.0,\
		camera_3d.get_viewport().size.y / 2.0,\
		0.0)
...
const PATH = "res://exp_shader/"
const MATERIAL_OFF = preload(PATH +"shader_material_off.gdshader")
const PIXEL = preload(PATH + "shader_pixel.gdshader")
const EDGE = preload(PATH + "shader_edge.gdshader")
const EDGE_P = preload(PATH + "shader_edge_plus.gdshader")
const SHIFT = preload(PATH + "shader_color_shift.gdshader")
const CRTV = preload(PATH + "shader_crt_tv.gdshader")

var shaderarray:Array = [MATERIAL_OFF, PIXEL,\
	EDGE, EDGE_P, SHIFT, CRTV]
var pos:int=0
...
func shader_switch():
	...
	if Input.is_action_just_pressed("1"):
		...
		pos = pos - 1
		if pos < 0:
			pos = shaderarray.size()-1
	elif Input.is_action_just_pressed("2"):
		changed = true
		pos = pos + 1
		if pos >= shaderarray.size():
			pos = 0

	if changed:
		shader_layer.material.shader = shaderarray[pos]
